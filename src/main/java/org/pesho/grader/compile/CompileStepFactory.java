...
public class CompileStepFactory {
	...
	public static CompileStep getInstance(File sourceFile, File graderDir, Map<String, Double> time, Map<String, Integer> memory, boolean isSubmission) {
		...
		if (sourceFile.getName().endsWith(ZipCompileStep.SOURCE_FILE_ENDING) && isSubmission) {
			return new ZipCompileStep(sourceFile, graderDir, time, memory);
		}
		if (sourceFile.getName().endsWith(ZipCompileStep.SOURCE_FILE_ENDING) && !isSubmission) {
			return new ZipLaTexCompileStep(sourceFile, graderDir, time, memory);
		}
